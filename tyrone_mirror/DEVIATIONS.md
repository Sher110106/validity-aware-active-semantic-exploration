...
81. **Engineering pilot (scene 00069, seed 42, one stage) produced zero settled completions; the reservation model, not the pilot allocation, is the blocker (2026-09-21).** Authorized by the user to run unsupervised against a $18 engineering-pilot cap under a $2.50 allocation (`allocation_id=pilot-scene00069-seed42-20260921`). The run never reached a paid Gemini call, so no data exists to score under either `official_asp` or `validator_plus_unanimity_raw_tau_1_0` (`prospective_runtime/manifest.py`'s two supported policies) — the paired scientific-phase comparison was never in scope for this attempt and nothing was skipped in that sense.

    Two independent, code-verified findings, not one:

    - **`conservative_input_bound()` (`prospective_runtime/transport.py`) is one byte of canonical JSON per token**, applied to a request whose body includes base64-encoded scene images. Measured against the real scene-0/member-0 request (6 JPEGs, ~500KB each, plus the 146-line scene-graph YAML and the completion prompt, captured from the stopped run's own `stages/0/`): the byte bound is 4,104,516; the real input token count, from a free `countTokens` call against the same content, is 9,317. Overshoot ratio 440.5x. `reserve()` (`gemini_campaign/ledger.py`) charges the byte bound, not a real count, so it asked for roughly $3.08 of input alone against a $2.50 allocation before any HTTP call — `AccountingHalt("allocation exhausted")` is correct behavior against that number, not a bug. The real per-turn reservation at measured cost (`cost_microdollars(9317, 8192)`) is $0.0377. At real counts, the existing $2.50 pilot allocation and the $18/$160 caps all have wide headroom; the fix is measuring the request, not raising any cap. `RequestContext.trusted_input_token_bound`, `NativeTokenCounter`, and `validate_native_count` already exist for exactly this and are unused. Wiring a native count into `reserve()` is a change to a financial control and needs the user's sign-off; it was not made here.

    - **`Ledger.allocate()` has no release, deallocate, or cancel.** `_allocated_total()` sums every allocation ever created, settled or not, and `remaining_microusd = ceiling - allocated`. Confirmed against `gemini_campaign/ledger.py`: after this pilot's one allocation and zero settlements, `summary()` reports `allocated_microusd: 2_500_000` and `remaining_microusd: 187_500_000` against a $190 ceiling — the $2.50 is gone from the ceiling permanently, with zero dollars ever spent. Each abandoned attempt (this campaign has already had two: the first killed by an `RLException` from `dataset_name`/`config_path` coupling, the second by the seed bug below) is a small, irreversible debit against the $190 ceiling regardless of outcome. This is a design question for the user, not something fixed here.

    Also fixed in this attempt, unrelated to the above: `prospective_integration/overlay_runtime.py`'s `deterministic_seed()` masked its SHA-256-derived seed to unsigned 32 bits (`int.from_bytes(..., "big")`), but the API requires a signed 31-bit seed (`0 <= seed <= 2**31-1`, enforced in `transport.py`'s `_validate_request`, which runs before any reservation). About half of live values overflowed; roughly half of the 8 concurrent ensemble-member workers failed with `seed must be an explicit deterministic integer` before touching the ledger — zero cost impact, confirmed via the run's own log and the ledger export. Fixed with `& 0x7FFFFFFF`; masking changes every derived seed, but nothing has ever settled under the old derivation, so there is no continuity to preserve. A pinned regression test (`test_deterministic_seed_masks_a_known_overflowing_hash`, `tests/test_overlay_runtime.py`) fixes one concrete input whose unmasked hash is known to overflow (`2321322531 > 2**31-1`); the prior test only sampled one random value, which has roughly even odds of missing the bug and did.

    State at stop: ledger `settled_microusd: 0`, `reserved_microusd: 0`, `unresolved_requests: 0` — no money was ever at risk. The pinned checkout (`active_semantic_perception`) is confirmed byte-identical against a pre-pilot hash snapshot for every `.py`/`.yaml`/`.launch` file; the only difference is one new file, `mapping/hydra/output/prospective_pilot_20260921/hydra_config.yaml` — hydra's own output snapshot, written to the isolated `dataset_name` path chosen for this attempt, matching the same output-writing behavior already present for `realsense`/`realsense_frontier` before this pilot ever ran. Two orphaned zombie processes (`Xvfb`, `roslaunch`, both state `Z`) from the crashed run were reaped on tyrone; display `:99` (the paused matrix run's own display) was not touched. No paid Gemini call occurred at any point in this campaign to date.


82. **Real-token-count reservation fix implemented and tested locally, pending advisor review before any tyrone deployment (2026-09-21/22, overnight, user-authorized unsupervised continuation).** Addresses #81's byte-bound-overshoot finding. Added `count_tokens()`/`native_input_bound()` to `prospective_runtime/transport.py`: a free, read-only call to the countTokens endpoint (`{"generateContentRequest": {"model": "models/gemini-3.8-flash", "contents": ..., "tools": ...}}`, no `generationConfig` — confirmed live #81 that an unwrapped `tools` field is rejected), falling back to the existing `conservative_input_bound()` byte bound on any failure (bad credential, transport error, or malformed/invalid response) — the fallback direction only ever over-reserves, never under-reserves, so it is the correct fail-closed behavior.

    Wired as an optional `input_bound_fn` parameter (default `conservative_input_bound`, i.e. unchanged behavior) through `prospective_runtime/author.py`'s `run_author_turns()` and `prospective_integration/author_overlay.py`'s `run_completion_turns()`/`run_single_turn()`. A new `prospective_integration/overlay_runtime.py::build_input_bound_fn()` returns the plain byte-bound function unchanged unless the environment variable `ASP_PROSPECTIVE_NATIVE_COUNT=1` is set, in which case it returns a closure using the real ledger context's credential. The deployed sibling overlay file (`prospective_integration/overlay/exploration/scripts/llm_completion.py`) now passes this into both of its Gemini-calling methods. `Ledger.reserve()`/`Ledger.settle()` (`gemini_campaign/ledger.py`) are untouched — this only changes what value the caller passes as `input_bound`, which `transport.conservative_input_bound()`'s own docstring already names as the intended extension point ("a caller with a trusted native token count should use that instead").

    14 new offline unit tests added (`tests/test_prospective_runtime.py`, `tests/test_author_overlay.py`, `tests/test_overlay_runtime.py`): exact request shape sent to countTokens, credential/response validation, fallback-on-any-failure (transport error, malformed response, bad credential — never a partial trust of a bad value), the opt-in flag's off/on/malformed-value behavior, and that the wiring genuinely reaches a caller-supplied `input_bound_fn` rather than silently ignoring it. Full suite: 160 tests, all green except the one pre-existing local-Mac-only missing-PyYAML skip unrelated to this change.

    Not yet deployed to tyrone; not yet run against any real ledger. An independent Opus design review was requested before any deployment or real spend, per this session's established judgment-call pattern — its outcome and (if approved) the actual pilot re-run's real numbers are recorded in the next entry.


83. **Advisor review: GO, conditional on three required changes; all three implemented, tested, and verified against real money (2026-09-22).** The independent Opus review of #82's fix found it directionally correct but flagged that `countTokens` is a provider *estimate*, not proven to equal the real `generateContent` `promptTokenCount` — unlike the always-safe byte bound, an under-count was not proven safe, and `Ledger.settle()` refusing to settle when real cost exceeds the reservation would leave real spend `DISPATCHED` and never priced, worst-case on a `MAX_TOKENS` response with zero output slack. Required: (1) a margin on the native count, capped by the byte bound — implemented as `min(count + max(256, count // 8), byte_bound)`; (2) full logging of every count and every fallback with its exception, since a silent fallback from a transient `countTokens` failure would be indistinguishable from "the fix doesn't work" — implemented as `prospective_integration/count_log.py`, wired via `ASP_PROSPECTIVE_COUNT_LOG`; (3) one cheap real call, before trusting the full ensemble, to directly compare `countTokens`'s count against the real settled `promptTokenCount` for the same request. Also applied: a guard in `count_tokens()` rejecting any `generationConfig` field outside the five fields already confirmed token-irrelevant, so a future change to that approved shape fails loudly instead of silently under-counting. 10 more tests; full suite 170, same single pre-existing unrelated skip. Committed `f07c85a`, synced to tyrone (`git bundle` → `git merge --ff-only`), and redeployed to the sibling checkout via the existing hash-verified `deploy.py` path (old file backed up as `llm_completion.py.before_native_count_fix_20260922`; new hash `3ed58d3a...`, matches `manifest.json`). Pinned checkout re-confirmed byte-identical against the 2026-09-21 hash snapshot.

    **Required change #3's verification call**, run for real against a new, isolated $1.00 allocation (`native-count-verify-20260922`, campaign `native-count-verify`, its own new ledger file — never touches the 2026-09-21 pilot's ledger): reused the real scene-0/member-0 image set and YAML already on disk from that stopped attempt (no new capture). First attempt used a deliberately tiny `max_output_tokens=64` to stress-test the zero-output-slack case; it hit a genuinely different, previously-unknown finding instead (below) and never settled — cost: one reservation retained unresolved at $0.0081, by design, never released, matching "no hidden retries." A second attempt with `max_output_tokens=16000` (matching the real pilot's configured value, not the 8192 default) settled cleanly: `finishReason: STOP`, real settled `promptTokenCount: 9317`, **exactly matching** the `countTokens` measurement of `9317` from #81/#82 — zero discrepancy, on the identical real request, checked twice (the first, truncated attempt's raw response also carried `promptTokenCount: 9317` in its `usageMetadata`, before the parser rejected it for the unrelated reason below). First real settled spend on this specific fix: $0.0163. Ledger after both attempts: `settled_microusd: 16292`, `reserved_microusd: 8101` (the one retained-unresolved reservation from attempt 1), `unresolved_requests: 1`, `allocated_microusd: 1,000,000` — all exactly as expected, nothing anomalous.

    **New, unrelated finding from the first (truncated) verification attempt**: with `max_output_tokens=64`, the real response's `usageMetadata` showed `thoughtsTokenCount: 121` and `candidatesTokenCount` omitted (i.e. 0) — **thinking-token usage alone exceeded the requested output cap**, tripping `transport._parse()`'s `candidate_tokens + thoughts > max_output` guard (`ValueError("candidate and thought tokens exceed maxOutputTokens")`), which `generate()` reports as a generic unresolved failure. This is a genuine model-behavior fact, not a bug in this campaign's code: `thinkingLevel: MEDIUM` apparently has some minimum thinking-token floor that is not itself bounded by a very small `maxOutputTokens`. Irrelevant at the real pilot's `maxOutputTokens=16000` (121 ≪ 16000), but worth recording: any future use of a small output cap (e.g. a cheap diagnostic probe) should stay comfortably above this floor, and any real ensemble member that happens to produce an unusually large thought count relative to its own cap could hit this same guard and go unresolved — the fix's own margin change does not protect against this, because it is an output-side effect, not an input-bound one.

    **Also confirmed, structural, not a defect**: `Ledger.allocate()`'s ceiling/phase-cap enforcement is scoped to its own SQLite file (`_allocated_total()` sums only `self.db`'s own `allocations` table), and `docs/prospective_overlay/README.md` explicitly documents `ASP_PROSPECTIVE_LEDGER_PATH` as "the run's isolated SQLite ledger file" — confirming this is the intended design, not an oversight. Consequence: the stated $190/$180 code ceilings are enforced *per ledger file*, not once globally across the whole campaign; nothing in code stops the sum across all of this campaign's separate ledger files (capability-probe, the 2026-09-21 pilot, this verification run, and the real pilot below) from exceeding $200 except the operator tracking it by hand. Real aggregate allocated across every ledger file this campaign has ever created, including the real pilot allocation below: approximately $2.00 (probe) + $2.50 (2026-09-21 pilot) + $1.00 (this verification) + $18.00 (the pilot below) ≈ $23.50 of the $200 user cap — comfortably within bounds, but the mechanism itself does not enforce that; a person or agent must.


84. **Real pilot re-run with the fix enabled: it works, and a second real bug surfaced — Gemini `responseId` values that start with `-` can never settle (2026-09-22).** The fork carrying out #82/#83 was cut off mid-run by a Claude API rate limit, after reporting only that it had "stopped the pilot now" against an infinite-but-cost-neutral retry loop; this entry is that stop's aftermath, reconstructed and verified directly against tyrone by the parent session, not by the fork.

    New isolated run `pilot_scene00069_seed42_native_count_20260922`, $18.00 allocation, fix enabled. **The fix works on real data.** 29 real completions settled cleanly across two stages of scene 00069 (8 ensemble members, several tool-loop turns each): real per-turn input tokens measured 13,000–15,000+ once tool-call history had accumulated (turn 0 matches #81/#83's 9,317; later turns grow with conversation history, as expected), real per-turn settled cost $0.0096–$0.0207. Officially settled: $0.448327 — trivial against the $18 allocation and the $190 ceiling. `native_count_verify_20260922` (#83) also has one real call, `verify-attempt`, sharing the exact same failure mode below.

    **A second, independent real bug.** 8 real calls in the pilot ledger (9 counting `verify-attempt`) ended `DISPATCHED`, never `SETTLED`, logged live as `ERROR: A parallel job failed: Gemini attempt unresolved`. Diagnosis (raw-archive replay against a disposable copy of the ledger file — the live one was never touched): every one of these calls actually succeeded — real `finishReason: STOP`, valid `usageMetadata`; `transport._parse()` accepts every one of them cleanly, replayed offline. Calling `Ledger.settle()` against a disposable copy with the exact real values reproduces the true cause: `LedgerError("provider response is invalid")`, from `_id()`'s regex `^[A-Za-z0-9][A-Za-z0-9_.:/-]{0,127}$` applied to `provider_response_id`. Real Gemini `responseId` values can start with `-` (observed: `-NWwasf4MsGlg8UP69rl8QU`, `-NOwat7-N47Eg8UP657DyQU`, and others) — a character the regex permits *inside* an identifier but rejects as its first character. 9 of 38 real calls tonight (≈24%) hit this — not a rare edge case. Recomputing `cost_microdollars()` by hand from each call's real usage (never written to the ledger, since `settle()` refused before recording anything) puts the true additional spend at **$0.101445** across the 8 pilot-ledger calls — real total spend for that run is therefore **≈$0.5498**, not the $0.448327 the ledger itself reports. Still trivial against the $18 allocation, but the ledger's own settled figure silently undercounts real spend for as long as this regex stands.

    **This compounds with #81's seed fix in a new way.** Once a call goes `DISPATCHED`-then-`unresolved` for *any* reason (this bug, a timeout, a network blip — not specific to responseId), the pinned pipeline's own automatic whole-stage-retry-on-any-failure logic reruns that member from turn 0 using the *same* deterministic `attempt_id`. `reserve()`'s `UNIQUE(campaign_id, attempt_id)` constraint then rejects every retry instantly with `AccountingHalt("request identity is already used")` — confirmed live in the pipeline log as ten consecutive `[ERROR] ... request identity is already used. Retrying...` lines. Cost-neutral (the constraint fires in `reserve()`, before any HTTP call), but that member can never progress again in that run, for any transient cause. `RequestContext.retry_of` already exists in the schema for a provenance-tracked fresh attempt and is unused by the pipeline glue. The fork stopped the pilot rather than let this loop run — the right call.

    **Neither bug is fixed here.** Both are financial-control identity/idempotency semantics (`_id()`'s regex is shared by nine different field types across the ledger, not just `provider_response_id`; `attempt_id` uniqueness is the ledger's core double-settlement defense) and need real review, not a rushed unsupervised patch at this hour. Left exactly as found: all 9 unresolved requests remain `DISPATCHED`, their reservations retained, nothing retried, nothing settled by hand.

    Verification before stopping for the night: pinned checkout re-hashed against the original pre-pilot baseline (`/tmp/pinned_before_pilot.sha256` on the tyrone host) — no new or modified `.py`/`.yaml`/`.launch` file beyond the one already documented in #81; the only diff lines are reordering among files that already shared identical hashes (e.g. empty `__init__.py`s), not a real difference. No live (non-zombie) process remained on tyrone (confirmed via `/proc/<pid>/status` state checks, not just `ps`); the run's own zombies were reaped, `/tmp` scratch scripts removed on both host and container. Display `:99` (the paused matrix run's own) was never touched. Aggregate allocated across every ledger file this campaign has ever created to date: ≈$23.50 of the $200 user cap.


85. **Both #84 bugs fixed, independently reviewed, and deployed; the 9 real completions #84 found stuck are now settled for real, at zero new spend (2026-09-22).** User authorized the fixes and the real scientific-phase run explicitly ("fix and all... come back with final results of the 200 budget actual run").

    `gemini_campaign/ledger.py`: added `_provider_response_id` (regex `^[A-Za-z0-9_.:/-]{1,128}$`, no leading-character restriction), used only for `provider_response_id` in `settle()` — every self-generated identifier keeps the original stricter `_id()`. Added `Ledger.resolve_attempt_id(campaign_id, base_attempt_id)`: a read-only pre-check that walks a `:retryN` suffix when `base_attempt_id` is already used, returning the prior request's id as `retry_of`; `reserve()`'s own `UNIQUE(campaign_id, attempt_id)` stays the real backstop. Wired into `run_author_turns`/`run_single_turn` via `getattr(transport.broker, "resolve_attempt_id", None)` — any broker without it (existing test doubles) keeps today's exact behavior.

    **Independent design review (fresh Opus reviewer, no relation to earlier ones) found and reproduced a real gap before deployment**: the pipeline restarts a whole member from turn 0, not just its failed turn, so a member whose *earlier* turn already settled gets that turn genuinely re-sent and re-billed — real work, not a cache-hit — on every restart, with no bound. Fixed with `Ledger.MAX_ATTEMPT_RETRY_GENERATIONS = 4`: `resolve_attempt_id` now raises `AccountingHalt` past 4 retry generations instead of continuing forever, and `summary()` gained `redundant_settled_microusd` (`SUM(settled_microusd) WHERE retry_of IS NOT NULL`) so re-billed spend is visible, never silently folded into ordinary settlement. New end-to-end regression test reproduces the reviewer's exact scenario against a real `Ledger`+`LedgerBudgetBroker`+`run_author_turns`, asserting it now succeeds up to the cap and fails loud past it. Full suite after: 198 tests (177 local/`tests/` + 21 `gemini_campaign/tests/`), all green on both the Mac and tyrone's real Python 3.9 container (179 there — 2 fewer skips, PyYAML is present).

    Committed `3321dea`, synced to tyrone via `git bundle`/`git merge --ff-only` (no overlay redeploy needed — none of this touched the deployed `llm_completion.py` file, only the underlying packages it imports).

    **Then reconciled #84's exact finding for real, using the actual fix, at zero new spend**: replayed each of the 8 real archived responses from `pilot_scene00069_seed42_native_count_20260922` through the live `Ledger.settle()` (not a disposable copy this time — these are genuinely already-incurred calls being correctly recorded, not new work). All 8 settled cleanly. That ledger's final state: `settled_microusd: 549772` ($0.549772, matching #84's hand-computed estimate exactly), `unresolved_requests: 0`, `redundant_settled_microusd: 0`. `native_count_verify_20260922`'s one remaining unresolved request (`verify-attempt`) was deliberately left alone — it is `#83`'s already-documented, unrelated, correctly-fail-closed finding (`thoughtsTokenCount: 121` exceeded that attempt's deliberately tiny `max_output_tokens: 64`), not the responseId bug, and forcing it to settle would misrepresent what actually happened.


86. **matrix_supervisor daemon found running again before the scientific campaign launch, due to a raw `kill -TERM` (not `systemctl stop`) earlier tonight; stopped correctly this time (2026-09-22).** Before launching the real scientific-phase campaign, a pre-flight check found the daemon active again (`systemctl --user status asp-matrix-supervisor.service`: "Active: active (running) since Mon 2026-09-21 10:26:29 IST... Scheduled restart job, restart counter is at 1"). Root cause: the daemon is installed as a systemd user service with `restart-on-failure` (per its own `SPEC.md`), and an earlier stop tonight used a raw `kill -TERM` on the process PID rather than `systemctl --user stop` — systemd's restart policy treats a signal-killed process as a failure and relaunched it roughly 5h40m before this check.

    Its own `status.json` confirmed it had done nothing harmful in the interim: `"phase": "paused"`, `"proposed_action": "validate_complete"`, `"pipeline_pids": []` — no active launch, just its own periodic read-only health polling (GPU/disk/process checks) against the shared `asp-noetic` container. Real risk, not touched: its own documented safety rule ("if container GPU fails twice, perform one full Docker stop/start cycle") could have autonomously restarted the container mid-block if a health check ever misread the campaign's own heavy GPU usage as a failure.

    Snapshotted current state to a new, isolated directory (`runs/matrix_supervisor/snapshots/before_scientific_campaign_20260922/`, preserving the existing `before_stop_20260921_045614/` snapshot untouched) and stopped it via `systemctl --user stop asp-matrix-supervisor.service` this time — confirmed `inactive (dead)` and, after a 15s wait, still `inactive` (an administrative stop does not trigger the restart-on-failure policy, unlike a raw kill). Its blocker (the unrecognized traceback on `scene00853_seed42_matrix120m`) was not touched or evaluated, per the standing instruction to leave that scientific judgment call alone. The five leftover zombie processes from tonight's earlier pilot runs (`Xvfb`/`roslaunch`/`hydra_ros_node`, PIDs 912/1368/231312/231340/232194, all confirmed state `Z`/`Zs`/`<defunct>`) remain exactly as before — harmless, already-dead process-table entries, left alone rather than force-reaped again for no reason.

    **Restart it correctly, if this comes up again**: `systemctl --user stop asp-matrix-supervisor.service`, never a raw `kill` on its PID, or the restart-on-failure policy will bring it back within the hour regardless of intent.


87. **Scientific campaign, block 1 (scene 00069, seed 42): a zero-cost roslaunch startup race on the first attempt, fixed by a clean retry; second attempt is real and settling normally (2026-09-22).** First launch attempt: roslaunch's `task_server` (a REQUIRED node) died within ~2 seconds of its own init with `[/semantic_inference/embed]: lookup service failed with message [no provider]` — it tried to call `semantic_inference`'s embedding service before that node had finished loading its CLIP (`ViT-L/14`) and YOLO (`yoloe-v8l-seg.pt`) models and registered the service. Because `task_server` is required, roslaunch tore down the entire launch (`nodelet1-10` also died, exit 255, likely a related symptom of the same cascade). This is an internal roslaunch node-startup race, not something this campaign's own launch-script timing (the `sleep 30` before starting Xvfb/`exploration_pipeline.py`) controls or caused — `task_server` tries the service almost immediately after its own init, well before that external wait even completes.

    Ledger confirmed `settled_microusd: 0, reserved_microusd: 0` both before and after — the failure happened entirely before any reservation, zero cost, zero risk. Cleaned up (killed the surviving `Xvfb :90`, cleared its lock files, killed the stale per-block watchdog) and relaunched the exact same block fresh. Second attempt: no race, all four mapping nodes (`semantic_inference`, `task_server`, `clio_node`, `hydra_ros_node`) came up and stayed up, `exploration_pipeline.py` reached `SCANNING` → `PLANNING` → real DSG file copy for both graphs, and real Gemini completions began settling: 5 settled so far at $0.0132–$0.0173 each, matching the 2026-09-21 pilot's measured per-turn range exactly. This is the first real progress in the scientific phase.

    Not fixed and not treated as needing a fix: this looks like ordinary, occasional startup-timing variance in the pinned launch file (model loading time vs. roslaunch's own node-start ordering), not a bug introduced by this campaign. Treated the same way the very first-ever pilot attempt (2026-09-21, `config_path`/`dataset_name` `RLException`) was: a launch that died having made zero paid calls is safe to archive and retry, not a "hidden retry" in the financial sense that rule is about. If this recurs across multiple blocks, it's worth a real fix (e.g. `task_server` retrying its own service call a few times before giving up); one recurrence is not yet evidence of that.


88. **Real, serious bug found and fixed live: `make_context()` never disambiguated by pipeline stage, so every stage beyond the first silently exhausted the retry-generation cap on ordinary progress, not failures (2026-09-22).** While block 1 (scene 00069, seed 42) was running well past its startup race (#87) — real settlements climbing normally, real navigation distance accumulating (`total_path_length_meters: 3.075` after stage 1) — `Ledger.summary()`'s `redundant_settled_microusd` climbed to more than half of total settled ($2.15 of $4.35, later $3.83 of $6.17), with **no** corresponding `ERROR`/`Retrying`/`unresolved` lines anywhere in the pipeline log. That absence is what gave it away: #84's retry mechanism is supposed to fire only after a real failure, and there wasn't one.

    Root cause: `overlay_runtime.make_context()`'s `turn_id`/`attempt_id` were built only from `scene_index`/`ensemble_index`/`call` — never the pinned pipeline's own stage counter (`exploration_pipeline.py`'s `self.stage`, incremented once per SCAN→PLAN→NAVIGATE cycle). Every stage's `LLMCompletion` calls therefore reconstructed the *exact same* deterministic `attempt_id` as every prior stage. `resolve_attempt_id` (#84) correctly avoided every resulting collision by walking a `:retryN` suffix — confirmed by inspection: every single settled row for turns 0 through 17 across three stages had gone through `:retry1`, then `:retry2` as a fourth stage began — so nothing double-settled and no accounting broke, but it mislabeled entirely legitimate, brand-new per-stage work as retry-driven re-billing, and, critically, it was silently spending down `MAX_ATTEMPT_RETRY_GENERATIONS = 4` on ordinary progress rather than genuine transient failures. Left running, every member would have hit `AccountingHalt("...exceeded 4 retry generations...")` after stage 4 or 5 — nowhere near any real path-length checkpoint (three stages had covered barely 3 meters of the 25/50/75/120 m targets).

    Fixed by threading the real stage number through without touching the pinned checkout: `LLMCompletion.base_path` already equals `exploration_pipeline.py`'s `WORKING_DIRECTORY`, which the pinned file itself sets to `BASE_DIRECTORY/<self.stage>` — so `os.path.basename(self.base_path)` is the real stage number, readable from the overlay with zero pinned-file edits. `make_context()` now requires an explicit `pipeline_stage` and folds it into `turn_id`/`attempt_id` (`stage{N}-scene{i}-member{j}-{call}`), so every stage's ids are naturally distinct and `resolve_attempt_id`'s retry path is reserved for genuine same-stage retries again. New regression test (`test_make_context_gives_every_pipeline_stage_a_distinct_attempt_id`) pins three different stage numbers to three different attempt_ids for the same scene/member/call. Full suite: 178 `tests/` (same one pre-existing unrelated skip) + 21 `gemini_campaign/tests/`, all green.

    Committed `904999e`, synced to tyrone, and redeployed to the sibling checkout via the hash-verified `deploy.py` path (old file backed up as `llm_completion.py.before_stage_fix_20260922`; new hash `ba695997...`, matches `manifest.json`). Also fixed `stop_scientific_block.sh`'s cleanup list, which was missing `semantic_inference`/`clio_node`/`nvblox_node` and left one process lingering after the first stop.

    Block 1's first attempt (`block_00069_42`) is stopped and preserved untouched as a historical record — real, correctly-priced work (`settled_microusd: 6,168,957`, `unresolved_requests: 0`, clean stop, zero money lost or at risk), just mislabeled by the pre-fix accounting. It is not being resumed or overwritten; a fresh attempt (`block_00069_42_v2`) starts clean under the fix. Pinned checkout re-verified byte-identical against the original baseline before relaunching.

    **This is the second real, independent bug the same underlying design gap (deterministic ids with no failure-mode/retry-mode disambiguation) has produced tonight** — first the intra-stage retry-after-failure collision (#84), now the cross-stage naming collision (#88). Worth flagging as a pattern for any future extension of this attempt-id scheme: any new dimension that can legitimately recur (a new stage, a new phase, a new anything) needs to be in the id from the start, not discovered live.


89. **Offline scoring tooling (`asp_offline.validator`, `prospective_runtime.policy`) does not accept this pipeline's real edge format out of the box; not fixed, scoped out for now (2026-09-22).** Attempted to run the zero-additional-cost `official_asp`/`validator_plus_unanimity_raw_tau_1_0` scoring against block 1's real, already-settled stage-0 data (`habitat_scene_graph_original_graph{0,1}.yaml`, `habitat_scene_graph_new_graph_{0..7}.yaml`). Every member was rejected by `validate_completion` with `ValidationIssue(code='edge_schema', message='edges must be a list of mappings')`.

    Cause: the pinned pipeline's own YAML edge format is a flow-style pair, `edges: [[0, 12], [2, 15], ...]` (`parse_response()` wraps each with `FlowList`, confirmed in both the observed and completion files) — but `asp_offline.validator._edges()` requires `edges` to be a list of Mappings (dicts), rejecting a plain `[a, b]` pair outright. This is a real format mismatch between the scoring module and this campaign's actual data, not a data-quality problem with the real completions themselves (the underlying graph content is fine; only the schema check rejects it).

    Not fixed here — this is a separate, real piece of engineering (either `asp_offline.validator` needs an edge-format adapter, or the pipeline's edges need conversion before scoring) and is out of scope for tonight's priority, which is running the real campaign blocks. `prospective_runtime.policy`'s `apply_unanimity_policy`/`process_cards` were presumably built and tested against a different (dict-shaped) edge fixture and never exercised against this pipeline's actual output until now. Left for a future session; the real graph data itself is preserved and available to score once this is fixed.

    **Follow-up, same night, after the campaign finished**: wrote a local edge-format adapter (`{"source": a, "target": b}` from each `[a, b]` pair, applied only at the scoring script's boundary, `asp_offline` itself untouched) and re-ran against block 1's real stage-0 data. Parsing now succeeds — no more `edge_schema` rejections — but every single predicted node is then removed as geometrically invalid under both `official_asp` and `validator_plus_unanimity_raw_tau_1_0`, for every member, in every track. Root cause, found by reading `asp_offline/models.py::Node.from_mapping`: it looks for a node's size under `"dimensions"`, `"size"`, or `"extent"`; this pipeline's real field is `"dimension"` (singular) — never matched, so `dimensions` resolves to `None` for every node, `aabb()` has nothing to compute from, and every predicted node fails validation for lack of usable geometry. A second, independent problem sits underneath even that: this pipeline's real `dimension`/`position`/`orientation` values are themselves YAML strings (`dimension: '[0.832, 0.599, 0.667]'`), not actual YAML lists (`[0.832, 0.599, 0.667]`) — `Node.from_mapping`'s own `vec()` helper requires `isinstance(v, (list, tuple))` and returns `None` for a string, so even fixing the field-name mismatch alone would not be enough; the stringified values need parsing (e.g. `ast.literal_eval`) first.

    Not fixed — this is now a confirmed, multi-layered schema mismatch (edge shape, node-size field name, and stringified numeric fields), not a one-line patch, and genuinely deserves its own dedicated pass rather than a third rushed fix appended to an already long, high-stakes night. `asp_offline`/`prospective_runtime.policy` were evidently built and unit-tested against a schema that never matched this pipeline's real YAML output on any of these three points. The real graph data itself (all 9 blocks' stage-0/1 completions) is preserved and fully available to score once a proper adapter (or a fix inside `asp_offline.models.Node.from_mapping`) exists.


90. **Same category of bug, one level up: `make_context()` still didn't disambiguate by block/run, so block 2 collided with block 1 the moment both reached stage 0 (2026-09-22).** Caught within minutes of block 2 (scene 00069, seed 43) starting: `redundant_settled_microusd` jumped again ($3,831,979 → $4,602,280) with no corresponding failure in the log — the identical signature as #88. Cause: this campaign deliberately shares one `campaign_id` across all 9 blocks in one ledger file (so the file's ceiling enforces the true $160 total globally, per #83), but `make_context()`'s #88 fix only added the pipeline stage number — `scene0-member0-completion` at stage 0 was still identical for block 2 as for block 1, and `resolve_attempt_id` "resolved" the collision the same mislabeling way.

    Fixed the same way, one level up: `make_context()` now also folds in `ledger_context.run_id` (`ASP_PROSPECTIVE_RUN_ID`, set fresh per block by `run_scientific_block.sh` as `run-<BLOCK_NAME>`) — the exact per-block dimension that was still missing. New regression test (`test_make_context_gives_every_run_a_distinct_attempt_id_even_with_a_shared_campaign_id`) pins two different run_ids under the same campaign_id to two different attempt_ids for the same scene/member/stage/call. Full suite: 179 `tests/` (same one pre-existing unrelated skip) + 21 `gemini_campaign/tests/`, all green.

    Block 2's first attempt (`block_00069_43`) stopped immediately on catching this (only stage 0 reached, `settled_microusd` contribution real but partly mislabeled same as #88) and preserved untouched, same treatment as block 1's first attempt. Fix committed, synced, and redeployed via the hash-verified path before any further block launches. **Lesson, now stated plainly for real this time**: every dimension that can legitimately vary and recur — stage, block/run, and whatever comes next — has to be in the attempt_id from the start. Two live incidents in one night finding this the hard way is enough; the next thing this scheme is extended to (if anything) should get this checked before launch, not after.

91. **Scientific block `block_00069_42_v6` stopped safely at 19.5 m when the shared allocation's reservation budget, not its settled spend, was exhausted (2026-09-22).** Three preceding launch attempts (`v3`–`v5`) failed before any paid request: strict ROS environment initialization (`ROS_DISTRO`/`ROS_MASTER_URI` under `set -u`) and then Hydra's default `realsense` output path. Each directory is preserved. `v6` used a run-local ROS launch wrapper that passed a unique Hydra `log_path`, reached real SCANNING → PLANNING, and produced valid stage artifacts through stage 6 plus a partial stage 7.

    The run made no hidden or unresolved spend: the shared ledger finished with `allocated_microusd=160000000`, `settled_microusd=36783435` ($36.783435), `reserved_microusd=0`, `unresolved_requests=0`, and `redundant_settled_microusd=4999102` unchanged from the pre-run baseline. However, `Ledger.reserve()` enforces the allocation against the **sum of every request's reservation**, and settled reservations are intentionally not released. At stop time, the allocation had accumulated `159976812` microUSD of reservations across 2,233 settled requests, even though only $36.783435 had actually settled. The pipeline then repeatedly logged `AccountingHalt("allocation exhausted")`; those retries performed no provider calls. The run was stopped before the watchdog's stall timeout, preserving all artifacts.

    This is a financial-control design constraint, not permission to raise a cap or mutate the ledger. The remaining aggregate code headroom is only $6.50 for a new allocation, so no further paid block is authorized under the current rules. A future continuation needs an independently reviewed reservation/release design (or a fresh user-approved budget), not a hand-edited ledger. The fixed offline scorer ran at zero API cost against v6's preserved stages and wrote `offline_scoring_20260922/block_00069_42_v6/summary.json`; stages 0–6 have both four-member tracks, while stage 7 is explicitly partial with all completion files missing. Pinned-checkout verification against `/tmp/pinned_before_120m_20260922.sha256` passed for all 616 files; `matrix_supervisor` remained inactive and no campaign processes remain.

92. **Zero-cost full analysis of the partial Gemini block completed; trajectory claims remain capped at stage 6 (2026-09-22).** Using the fixed scorer (`5c178e2`), the author-faithful evaluator, extension-policy replay, LOSO-calibrated decision replay, validator issue taxonomy, raw prompt archive, and read-only ledger were all run against the preserved `block_00069_42_v6` artifacts. The tracked graph reached 19.5 m at P=0.692, R=0.336, F1=0.452, normalized GED=1.109; 0–19.5 m path-normalized AUC was P=0.757, R=0.184, F1=0.283, normalized GED=1.119. The static completion replay's best strict support policy (filter + raw-support τ=1.0) reached object P=0.769, R=0.184, F1=0.284, normalized GED=0.937 AUC. These completion numbers are prediction-quality replays, not counterfactual live trajectories.

    Of 55 available completion files, all 55 were accepted; 35 predicted nodes were removed (30 AABB conflicts, 5 impassable crossings). Stage 2 scene 0 member 0 is missing; stage 7 has all eight member files missing and no navigation checkpoint. Decision replay's null control matched the official selected pose exactly at all seven completed stages. Filter-only moved one stage (mean 0.185 m); calibrated support moved five (mean 2.559 m, max 5.920 m); combined risk moved 0/7 stages at β=0, 2/7 at β=1, and 6/7 at β=5/20. Detours and robot collisions remain unidentifiable.

    The v6 ledger contains 898 settled requests, all `STOP`, no errors/retries: $18.552664 settled against $65.749854 summed reservations. Raw archive and ledger agree on 20,401,514 input, 747,185 thought, 119,712 candidate, and 21,268,411 total tokens. Native-count instrumentation recorded 1,162 calls with zero fallbacks; real count mean 17,388 (9,346–24,099), safety-bound mean 19,561. At a common 16.425 m cap, v6 F1 AUC=0.256 versus earlier DeepSeek v3=0.318 and completed matrix=0.216; this is descriptive only because backend and checkpoint schedules differ. Full report and JSON artifacts are mirrored under `tyrone_mirror/runs/prospective_gemini/scientific_campaign_20260922/offline_analysis_20260922/block_00069_42_v6/` and summarized in `GEMINI_PARTIAL_ANALYSIS_2026-09-22.md`.
